    ...
    def forward(self, bert_output, startpos, endpos=None, position_embedding=True):
        ...
        for i in range(batch_size):
            ...
            for idx, time_start_seq in enumerate(time_start_list):
                ...
                # 复制每个时间段的查询值到时间段的长度
                all_time_qvec.append(attention_output.expand(output_shape[0], diff_val_list[idx], output_shape[2]).contiguous())
...
class GlobalEncoder(nn.Module):
    ...
    计算globalscore
    """
    def __init__(self, batch_size, seq_len, vec_size, num_layers=2, batch_first=True, bilstm=True, drop_prob=0.5) -> None:
        super().__init__()
        self.batch_size = batch_size
        self.seq_len = seq_len
        self.vec_size = vec_size
...
class Extract(nn.Module):
    def __init__(self, batch_size, seq_len, cated_vec_size, num_labels, num_layers=2, batch_first=True, bilstm=True, soft_label=False) -> None:
        super().__init__()
...
class PatientExtract(nn.Module):
    def __init__(self, batch_size, seq_len, hidden_size, num_labels, num_layers=2, batch_first=True, bilstm=True, soft_label=False) -> None:
        super().__init__()
        self.num_labels = num_labels
        self.soft_label = soft_label
        # self.training = training
    ...
    def forward(self, x, start_pos=None, end_pos=None, attention_mask=None, l1_h_0=None, l1_c_0=None, l2_h_0=None, l2_c_0=None):
        ...
        start_logits = torch.mul(start_logits_lstm, start_global_score.expand(start_logits_lstm_shape[0], start_logits_lstm_shape[1], start_logits_lstm_shape[2]))
...
class EventExtract(nn.Module):
    ...
    def forward(self, x, event_start_pos=None, event_end_pos=None,
        ...
        diagnosisconfirmed_output = self.diagnosisconfirmed(x, diagnosisconfirmed_start_pos, diagnosisconfirmed_end_pos, attention_mask)
        ...
        event_type_loss = event_output[0] + onset_output[0] + hospitalvisit_output[0] + diagnosisconfirmed_output[0] + inpatient_output[0] + discharge_output[0] + death_output[0] + observed_output[0]
        start_cat = torch.cat([event_output[1], onset_output[1], hospitalvisit_output[1], diagnosisconfirmed_output[1], inpatient_output[1], discharge_output[1], death_output[1], observed_output[1]], dim=-1)
        end_cat = torch.cat([event_output[2], onset_output[2], hospitalvisit_output[2], diagnosisconfirmed_output[2], inpatient_output[2], discharge_output[2], death_output[2], observed_output[2]], dim=-1)
        
        
        # event tuple
        date_output = self.date(torch.cat([x, start_cat, end_cat], dim=-1), date_start_pos, date_end_pos, attention_mask)
        symptom_output = self.symptom(torch.cat([x, start_cat, end_cat], dim=-1), symptom_start_pos, symptom_end_pos, attention_mask)
        labtest_output = self.labtest(torch.cat([x, start_cat, end_cat], dim=-1), labtest_start_pos, labtest_end_pos, attention_mask)
        imagingexamination_output = self.imagingexamination(torch.cat([x, start_cat, end_cat], dim=-1), imagingexamination_start_pos, imagingexamination_end_pos, attention_mask)
        location_output = self.location(torch.cat([x, start_cat, end_cat], dim=-1), location_start_pos, location_end_pos, attention_mask)
        spot_output = self.spot(torch.cat([x, start_cat, end_cat], dim=-1), spot_start_pos, spot_end_pos, attention_mask)
        vehicle_output = self.vehicle(torch.cat([x, start_cat, end_cat], dim=-1), vehicle_start_pos, vehicle_end_pos, attention_mask)
        
        event_tuple_loss = date_output[0] + symptom_output[0] + labtest_output[0] + imagingexamination_output[0] + location_output[0] + spot_output[0] + location_output[0] + vehicle_output[0]
...
class RelationExtract(nn.Module):
    ...
    def forward(self, x, socialtelation_start_pos=None, socialtelation_end_pos=None,
        ...
        # relation tuple
        localid_output = self.localid(torch.cat([x, socialtelation_output[1], socialtelation_output[2]], dim=-1), localid_start_pos, localid_end_pos, attention_mask)
        name_output = self.name(torch.cat([x, socialtelation_output[1], socialtelation_output[2]], dim=-1), name_start_pos, name_end_pos, attention_mask)
...
class JointExtract(nn.Module):
    def __init__(self, config, batch_size, model_path, seq_len, vec_size, cated_vec_size=None, num_layers=2, batch_first=True, bilstm=True) -> None:
        super().__init__()
        self.bilstm = bilstm
        self.batch_size = batch_size
        self.vec_size = vec_size
        ...
        self.patientextract = PatientExtract(batch_size, seq_len, vec_size, num_labels=7)

        self.relationextract = PatientExtract(batch_size, seq_len, vec_size, num_labels=4)
        self.eventextract = PatientExtract(batch_size, seq_len, vec_size, num_labels=16)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        """
        all_seq_vec = bert_feature_output
        patient_outputs = self.patientextract(all_seq_vec, patient_start_pos, attention_mask=attention_mask, l1_h_0=l1_h_0, l1_c_0=l1_c_0, l2_h_0=l2_h_0, l2_c_0=l2_c_0)
        ...
        """
        relation_outputs = self.relationextract(all_seq_vec, relation_start_pos,attention_mask=attention_mask, l1_h_0=l1_h_0, l1_c_0=l1_c_0, l2_h_0=l2_h_0, l2_c_0=l2_c_0)
        relation_loss = relation_outputs[0]
        relation_pos = relation_outputs[1:]
        print("relation_output")
        
        event_outputs = self.eventextract(all_seq_vec, event_start_pos,attention_mask=attention_mask, l1_h_0=l1_h_0, l1_c_0=l1_c_0, l2_h_0=l2_h_0, l2_c_0=l2_c_0)
        event_loss = event_outputs[0]
        event_pos = event_outputs[1:]
        print("event_output")
